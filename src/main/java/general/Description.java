...
public class Description {
  ...
  public Description(SimpleText.TextGenerator title, SimpleText.TextGenerator description,
      SimpleText.TextGenerator altDescription) {
    this.description = description;
    this.title = title == null ? null : () -> TextModifiers.blue + title.get();
    this.altDescription = altDescription == null ? null
        : () -> Game.get().getUserInputListener().isKeyPressed(GLFW_KEY_LEFT_ALT)
            ? altDescription.get() : null;
  }
  ...
  public TextBox getAsTextBox(int layer, SpriteBatching bs, float cost) {
    ...
    SimpleText costtxt = new SimpleText(TextModifiers.green + "cost: " + cost, "Calibri", 500, 0, 0,
        layer
        , 25, bs, "basic", null);
    texts.add(costtxt);
