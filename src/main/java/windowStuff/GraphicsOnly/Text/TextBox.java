...
import static GlobalUse.Constants.screenSize;
...
public class TextBox implements Text { //not sure if imgonna use this,
  ...
  private final ArrayList<SimpleText> texts;
  ...
  private void arrange() {
    if (!rearrange) {
      ...
      for (SimpleText text : texts) {
        ...
        background.setPosition(left + (left - right) / 2f,
            down + (up - down) / 2f - texts.get(texts.size() - 1).getFontSize() / 2);
