...
public final class Game implements UserInputHandler {
  ...
  public static final float tickIntervalMillis = 1000f / 60;
  public static final float secondsPerFrame = 1/60f;
  ...
  public void nuke() {
    ...
    for(TickDetect detect:tickables){
      detect.delete();
    }
    newTickables.clear();
    for(KeyboardDetect detect:keyDetects){
      detect.delete();
    }
    newKeyDetects.clear();
    for(MouseDetect detect:mouseDetects){
      detect.delete();
    }
