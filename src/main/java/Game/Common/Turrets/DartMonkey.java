...
public class DartMonkey extends Turret {
  ...
  protected ImageData getImageUpdate(){
    ...
    private final List<Button> upgradeButtons = new ArrayList<>(0);
  ...
  public void onGameTick(int tick) {
    ...
    protected Upgrade up010() {
        return new Upgrade("Rough", new Description("Material enhancements","Bloons hit are stunned by a rough surface"),
                () -> {
                    bulletLauncher.addMobCollide(this::stunPoison,0);
                }, 15);
    }
  @Override
  ...
  public boolean stunPoison(Projectile proj, TdMob mob){
      double duration= (proj.getStats()[Stats.power]+2)* 250 / Math.sqrt(mob.getStats()[TdMob.Stats.health]);
      mob.addBuff(new StatBuff<TdMob>(StatBuff.Type.MORE, (float)duration, TdMob.Stats.speed, 0));
      return true;
  }
    ...
    protected Upgrade up001() {
        return new Upgrade("sharper",
                ...
                () -> addBuff(new StatBuff<Turret>(StatBuff.Type.ADDED, Stats.power, 1)), 20);
    }
        ...
        return new Upgrade("ranger", new Description("Ranger","Settable range, When not attacking the dart monkey enters stealth",
                "when not having a target range is reduced by 50%, after 0.75 sec of not having a target gets 4 times damage for 2 seconds"),                () -> {
                    ...
                    var b = new DragableButton(new Sprite("Button", 100).setPosition(x, y).setSize(25, 25)
                        ...
                        return (X, Y) ->{
                        float range = getStats()[Stats.range];
                        this.addBuff(new StatBuff<Turret>(StatBuff.Type.ADDED, Stats.range,
                                (float) Math.sqrt(Util.distanceSquared(x - btn.getX(), y - btn.getY()))-range));
                    };
