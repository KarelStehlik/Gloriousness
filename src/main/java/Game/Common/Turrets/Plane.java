...
public class Plane extends Turret {
    ...
    public void onGameTick(int tick) {
        ...
        turnTowards(next.getX(), next.getY(), 1.7f);
        if (Util.distanceSquared(x - next.getX(), y - next.getY()) < 5000) {
    ...
    public void onStatsUpdate() {
        ...
        if (pineappleLauncher != null) {
            pineappleLauncher.setPierce((int)stats[ExtraStats.PineRadius]);
            ...
            pineappleLauncher.setSize(stats[ExtraStats.PineSize]);
    ...
    private void explodePine(Projectile pineapple) {
        world.aoeDamage(
                (int) pineapple.getX(), (int) pineapple.getY(), (int) pineapple.stats[Projectile.Stats.pierce],
                pineapple.getPower(),
                DamageType.TRUE);
        world.explosionVisual((int) pineapple.getX(), (int) pineapple.getY(),
                (int) pineapple.stats[Projectile.Stats.pierce], false, "Explosion2");
    }

    private static final long pineappleId = Util.getUid();
    private Buff<Turret> radiusbuff;
    private Buff<Turret> steroids;
    private Buff<Turret> sizeBuff;
    private boolean pineapplesActive = false;
    ...
    protected Upgrade up010() {
        return new Upgrade("pineapple", new Description("Pineapple", "Adds ability to drop pineapples and 3 extra pierce.",
                ...
                () -> {
                    ...
                    steroids = new StatBuff<>(Type.MORE, ExtraStats.PinePower, 3);
                    ...
                    Ability a = Ability.add("pineapple", 15000,
                            ...
                            () -> {
                                ...
                                addBuff(new DelayedTrigger<Turret>(1000, (Turret plan) -> {
                                    Plane plane = (Plane) plan;
                                    addBuff(sizeBuff);
                                    addBuff(steroids);
                                    addBuff(radiusbuff);
                                    onStatsUpdate();
                                    pineappleLauncher.move(x, y);
                                    pineappleLauncher.attack(0, false);
                                    removeBuff(sizeBuff);
                                    removeBuff(steroids);
                                    removeBuff(radiusbuff);
                                    onStatsUpdate();
                                    plane.pineapplesActive = false;
                                }
                                        , false
                                ));
  ...
  public static final class ExtraStats {
    ...
    public static final int PineSize = 15;
