...
public class Druid extends Turret {
    ...
    return new Upgrade("Button", () -> "bounces off walls",
        () -> {
          bulletLauncher.addProjectileModifier(p -> p.addBuff(
              new OnTickBuff<Projectile>(Float.POSITIVE_INFINITY, Projectile::bounce)));
        }, 200);
  }
  ...
  private Upgrade up200() {
    return new Upgrade("Button", () -> "regrows 4 more times",
        () -> {
          addBuff(
              new StatBuff<Turret>(Type.ADDED, Float.POSITIVE_INFINITY, ExtraStats.respawns, 4));
        }, 800);
  }
  ...
  private Upgrade up400() {
    return new Upgrade("Button", () -> "attacks way faster",
        () -> addBuff(new StatBuff<Turret>(Type.MORE, Float.POSITIVE_INFINITY, Stats.cd, 0.3f)),
        10000);
  }
  ...
  private Upgrade up500() {
    return new Upgrade("Button", () -> "keeps regrowing basically forever",
        () -> {
          addBuff(
              new StatBuff<Turret>(Type.ADDED, Float.POSITIVE_INFINITY, ExtraStats.respawns, 20));
        }, 30000);
  }
  ...
  private Upgrade up010() {
    return new Upgrade("Button", () -> "increases pierce when regrowing",
        () -> {
          addBuff(
              new StatBuff<Turret>(Type.ADDED, Float.POSITIVE_INFINITY, ExtraStats.pierceScaling,
                  .4f));
        }, 150);
  }
  ...
  private Upgrade up020() {
    return new Upgrade("Button", () -> "can hit the same bloon many times",
        () -> {
          bulletLauncher.addProjectileModifier(p -> p.addBuff(
              new OnTickBuff<Projectile>(Float.POSITIVE_INFINITY, Projectile::clearCollisions)));
        }, 500);
  }
    ...
    return new Upgrade("Button", () -> "Projectiles have more speed and piercs",
        () -> {
          bulletLauncher.addProjectileModifier(p -> {
            p.addBuff(
                new StatBuff<Projectile>(Type.MORE, Float.POSITIVE_INFINITY, Projectile.Stats.speed,
                    7));
            p.addBuff(new StatBuff<Projectile>(Type.MORE, Float.POSITIVE_INFINITY,
  ...
  private Upgrade up002() {
    return new Upgrade("Button", () -> "increases power and size when regrowing",
        () -> {
          addBuff(new StatBuff<Turret>(Type.ADDED, Float.POSITIVE_INFINITY, ExtraStats.powScaling,
              .4f));
          addBuff(new StatBuff<Turret>(Type.ADDED, Float.POSITIVE_INFINITY, ExtraStats.sizeScaling,
              .15f));
        }, 2000);
  }
