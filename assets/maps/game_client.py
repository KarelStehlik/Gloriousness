from pyglet.window.key import MOD_CTRL, MOD_ALT

from assets.maps.constants import screen
from imports import *
import groups
...
class Game:
    ...
    def mouse_press(self, x, y, button, modifiers):
        if modifiers & MOD_ALT:
            TrackObstacle(x, y, len(self.nodes), self)
            return
    ...
    def save(self):
        widthMod=1920/screen.width
        heightMod=1080/screen.height
        text = self.mapname
        for n in self.nodes:
            text+='|'
            text += n.save(widthMod,heightMod)
        with open("output.txt", "a") as file:
            file.write(text + "\n")

        from PIL import Image
        import math
        bg = Image.open(f"imageFiles/{self.mapname}.png").resize((screen.width, screen.height))
        width = 64
        track = Image.open("imageFiles/Track.png").resize((width, width))
        result = Image.new("RGB", (screen.width, screen.height), (255,) * 3)
        result.paste(bg, (0, 0))
        ...
        for n in self.nodes[1::]:
            ...
            for _ in range(int(distance_squared(*point, n.x, n.y)**.5 // speed)):
                point[0] += direction[0]
                point[1] += direction[1]
                result.paste(track, (int(point[0]-width//2),  screen.height - int(point[1]+width//2)), track)
            point = [n.x, n.y]
...
class TrackNode:
    ...
    def save(self,widthMod,heightMod):
        return f"Node {int(self.x*widthMod)},{int(self.y*heightMod)}"
    def isPoint(self):
        return True
...
class TrackObstacle:
    ...
    def __init__(self, x, y, order, game):
        self.x = x
        self.y = y
        self.sprite = pyglet.sprite.Sprite(images.images.redzone, x, y, batch=groups.g[7])
        self.sprite.scale = self.size / self.sprite.width
        self.id = order
        self.game = game
        game.nodes.append(self)
        ...
        self.text = pyglet.text.Label(str(order), x=self.x,
                                      ...
                                      anchor_x="center", align="center", anchor_y="center")
        self.selected = 3
        self.selX, self.selY = x, y

    def mouse_release(self, x, y, button, modifiers):
        if (button == 1)&(self.selected<3):
            self.selected = 0
    ...
    def mouse_click(self, x, y, button, modifiers):
        if(self.selected==3):
            self.sprite.width=abs(x-self.selX)
            self.sprite.height=abs(y-self.selY)
            self.x=self.selX-(self.selX-x)/2
            self.y=self.selY-(self.selY-y)/2
            self.selected=0
            return True
    ...
    def save(self,widthMod,heightMod):
        return f"Rect {int(self.x*widthMod)},{int(self.y*heightMod)},{int(self.sprite.width*widthMod/2)},{int(self.sprite.height*heightMod/2)}"
    def isPoint(self):
        return False